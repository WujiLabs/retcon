...
**Asst-text continuity — detection after the fact.** branch_context's most-recent assistant text is something we sent upstream and got back; claude appends it to its local jsonl on every successful turn. So in normal operation, that text MUST appear in claude's next `/v1/messages` body. If it doesn't, claude truncated past it — almost certainly via `/rewind`. retcon walks `branch_context` backward for the last assistant, walks claude's body for matching text, and on miss releases the fork (NULLs `branch_context_json`, emits `session.branch_context_released`). The check is skipped on the very first follow-up after rewind_to (when the anchor is the rewound target's response, possibly compacted out of claude's local view) — see "Fresh-fork skip" in IMPLEMENTATION.md.

**The release reminder — closes the loop on the way OUT.** When the asst-text check actually catches divergence, retcon also injects `<retcon-released>` reminder blocks into the very request that triggered the release. The AI sees them on the same turn it caused the release, includes the id of the last successfully fork-applied revision, and gets a propagation directive: "always include a brief mention in your response, regardless of whether the recipient is the user or — if you are a Task subagent — a parent agent reading your response as a tool result." Without this, the release was silent from the AI's POV — Anthropic returns 200, the AI never learns the fork is gone, the user sees the AI talk past prior fork content with no acknowledgment. Now the AI can guide the user back to `recall(turn_id="<last-fork-applied>")` to inspect or `rewind_to(turn_id="<last-fork-applied>", message=...)` to resume.
...
The first two halves landed in v0.5.3 / v0.5.4; the release-side reminder and the fresh-fork-skip refinement landed in v0.5.5 (after a forensic on session b17275fb showed every healthy fork was getting silently released within one turn — the asst-text anchor was the rewound target, which `/compact` had erased from claude's local view weeks earlier). Together they shift `/rewind` interop from "silent wrong answer or 400" to "user warned proactively, fork released cleanly if it does happen, AI guides the user back to the fork's anchor turn."

### Per-fork token detection vs pattern matching

A natural design question: how does retcon recognize "branch_context's tail is still the synthetic landing turn" without false-matching user content that happens to mention `<retcon-active>` literally? This codebase alone has dozens of such references in conversation history. A regex match would false-positive every time the user discusses retcon's own design.

The answer is a per-fork random token. Each `rewind_to`/`submit_file` generates a `tok_<12-hex>` (48 bits of entropy), embeds it as a `fork-id="..."` attribute on the synthetic turn's `<retcon-active>` opening tag, AND persists it to `sessions.branch_context_fork_id`. The detection is exact-equality match between the column and the tail user message's first text block — not a regex against an arbitrary token-shaped string. The DB column is the only authoritative source. A user could quote `<retcon-active fork-id="tok_aaaaaaaaaaaa">` in their message and it would not match if retcon's stored token was `tok_bbbbbbbbbbbb`.

The token is a retcon-internal marker. The AI never echoes it; the synthetic's reminder text explicitly tells the AI to ignore it. Mechanism in IMPLEMENTATION.md.
