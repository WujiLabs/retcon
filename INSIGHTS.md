...
The anchor token embedded in `rewind_to`'s tool_result (`<retcon-anchor token="tok_<12hex>" />`) is the per-fork handle that carries across turns. The proxy's body-scan finds it on the next `/v1/messages` and splices `target_messages_json` (held on the `fork_anchors` row) into the request. Mechanism in IMPLEMENTATION.md.
...
The mechanism (anchor token in tool_result, body-scan splice, fork_anchors state machine) lives in IMPLEMENTATION.md.

## Why /compact aligns the two realities

When you run `/clear` or `/compact` inside claude, the SessionStart hook fires with `source=clear` or `source=compact`. retcon marks any active `fork_anchors` row for the session as released (reason=`clear` or `compact`) and goes back to forwarding claude's body unchanged.
...
That LLM call is the key. It goes through our proxy like any other `/v1/messages`, which means **our anchor splice runs on it**. The conversation we're asking the model to summarize is the forked branch (claude's local body still carries the rewind_to tool_result with its anchor token, so the scan finds it), not claude's pre-fork view. So the summary that comes back represents the fork.
...
That leaves three indirect channels, and retcon uses all three:

**The activation reminder — proactive, primed once on the way IN.** When `rewind_to`/`submit_file` activates the fork, the synthetic landing turn carries `<retcon-active>` reminder blocks adjacent to the user's verbatim message. The AI in the rewound branch sees them and, because claude is trained on similar `<system-reminder>` shape, surfaces the warning to the user in its response: "while this fork is active, don't use claude code's `/rewind`." The AI's response is the only text from retcon's side that actually reaches claude code's UI for the human to read; pre-splice tool results are discarded by the splice itself.

**Anchor scan — detection after the fact.** The `rewind_to` / `submit_file` tool_result carries `<retcon-anchor token="tok_<12hex>" />` inside its text. On every `/v1/messages`, the proxy scans claude's outgoing body backward through user-role `tool_result` blocks looking for the latest anchor token. In normal operation the token is always there (the tool_result lives in claude's local jsonl from the moment the rewind was scheduled). When `/rewind` truncates past that turn, the token is gone, the scan returns null, retcon looks up the session's active `fork_anchors` row, marks it released (reason=`divergence`), and emits `session.fork_anchor_released`. Anchor token entropy is 48 bits — a user-pasted `<retcon-anchor token="...">` in plain message text won't false-match because the scan only inspects `tool_result` blocks, not user-typed content.

**The persistent release reminder — closes the loop on the way OUT.** When divergence (or any other release reason) fires, retcon injects `<retcon-released>` reminder blocks into the request body on EVERY subsequent turn until the AI acks. The reminder includes the `turn_id` of the last successfully fork-applied revision and an explicit MUTE directive: `recall(turn_id="<id>")` will silence the reminder. Without persistence, a release that fired on a turn the user / model didn't actually read would go unnoticed; with persistence, the alarm keeps surfacing until the AI demonstrates it has seen the released turn. No auto-mute, no silent reset.

The anchor approach has no equivalent of v0.5.x's "short ambiguous reply false-match" failure mode — the token is 48 bits of entropy per fork, not a content string. Cascade rewinds (rewinding to a turn that was itself created by an earlier rewind) work naturally because the backward scan picks the most-recent anchor; older anchors in body become historical text.

v0.5.3 / v0.5.4 shipped the asst-text continuity check; v0.5.5 added the release-side reminder; v0.6 replaced the continuity check entirely with the anchor scan after a forensic on session b17275fb showed the asst-text approach false-releasing every fork whose target had been compacted out of claude's local view. Together they shift `/rewind` interop from "silent wrong answer or 400" to "user warned proactively, fork released cleanly if it does happen, AI guides the user back to the fork's anchor turn."
...
- **/compact's summarization is a regular `/v1/messages` to `ANTHROPIC_BASE_URL`, with the existing conversation in `messages[]` and a "summarize..." user-role instruction APPENDED at the tail.** Two halves, both load-bearing.
  - *Routes through us* (not a side-channel endpoint): so `applyAnchorSplice` runs on the call.
