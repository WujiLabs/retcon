...
- [ ] **Disable persistent fork override post-`/compact`** — When the user runs `/compact` mid-fork, claude rebuilds its local jsonl with a summary that respects the forked context (since the proxy fed it the forked context all along). At that point the user's intent is "the conversation is now compact again, stop overriding." If we keep applying `branch_context_json` after compact, we lose the storage savings the user just bought back. Plan: install a `PostCompact` hook (assuming claude offers one — if not, sniff `/compact`-shaped commands or watch claude's body for a sudden length collapse) that clears `sessions.branch_context_json`. The penultimate-user algo only reads claude's tail so the forked branch is preserved up to that point in claude's view; clearing branch_context_json hands the wheel back to claude. Defer to v1.1.
