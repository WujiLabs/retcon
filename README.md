...
**Detection if you /rewind anyway.** retcon's anchor splice (v0.6) scans claude's outgoing body backward for `<retcon-anchor token="tok_<12hex>" />` inside the latest `tool_result` block. That token was embedded in the `rewind_to` / `submit_file` tool_result when the fork was created, so it lives in claude's local jsonl from the moment the rewind was scheduled. When `/rewind` truncates claude's jsonl past that turn, the token disappears, retcon detects the divergence, marks the active `fork_anchors` row as released (reason=`divergence`), and emits a `session.fork_anchor_released` audit event.

**Persistent release notice until you ack.** When the divergence guard fires, retcon injects a `<retcon-released>` reminder into the request body and keeps injecting it on every subsequent `/v1/messages` until the AI acks. The reminder names the `turn_id` of the last successfully fork-applied revision and tells the AI explicitly how to mute it: call `recall(turn_id="<last-fork-applied>")`. The AI can guide you back via `recall` to inspect, `rewind_to(turn_id="<last-fork-applied>", message="...")` to resume, or `dump_to_file(turn_id="<last-fork-applied>")` to inspect-and-edit before resuming. The release isn't silent and isn't a one-shot — it surfaces on every turn until the alarm is explicitly cleared.

The detection has no false-negative for short ambiguous replies (the v0.5.x asst-text continuity check would false-pass when both branches happened to have an "OK" assistant). The anchor token is 48 bits of entropy per fork; collisions are not a real concern.

Practical workflow: **once you give the AI control via retcon's `rewind_to`, let the AI steer until `/clear`, `/compact`, or another `rewind_to` runs.** If you need to manually rewind, do so BEFORE invoking retcon's tools, or release the fork via `/clear` first. If you do hit `/rewind` mid-fork, the AI tells you what happened and how to dive back in.
...
`retcon clean --actor <name>` wipes every row associated with that actor: events, branch_views, revisions, tasks, sessions, fork_anchors, and pending registrations. Defaults to dry-run; pass `--yes` to apply. Refuses to run while the daemon is up unless you pass `--force`.
...
- `confirm=<clean_token>` — the AI's own claim that the message stands alone. Server runs a narrow regex backstop on a 4-pattern list (catches "see above", "continue from here", "redo your last answer", "the last/previous question I asked") and either inserts the active `fork_anchors` row + returns the anchor-bearing tool_result, or rejects with a fresh token pair.
...
- [IMPLEMENTATION.md](./IMPLEMENTATION.md) — *how* the non-trivial mechanisms work. Anchor splice as the persistent-fork driver, SR pipeline across three time points, deferred fork.forked across tool_use chains, harness-injection skip in turn_back_n, state-divergence detection via anchor scan, the `<retcon-active>` reminder block, persistent `<retcon-released>` reminder, cache_control marker handling, resume binding, content-addressed body storage, event sourcing + projector chain.

If you're reading the code and want to understand why a piece is shaped a certain way, INSIGHTS.md. If you're tracing a specific pipeline (anchor splice, SR construction, fork_anchors state machine), IMPLEMENTATION.md.
