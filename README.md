...
**Detection if you /rewind anyway.** retcon's persistent-fork splice runs an asst-text continuity check before each turn: `branch_context`'s most-recent assistant text must appear somewhere in claude's outgoing body (claude assembles every upstream response into its local jsonl, so it's always present in normal operation). When `/rewind` truncates claude's jsonl past that point, the text disappears and retcon detects the divergence — NULLs `branch_context_json`, emits a `session.branch_context_released` audit event.

**Release notice on the way OUT.** When the divergence guard fires, retcon also injects a `<retcon-released>` reminder into the very request that triggered the release. The AI sees on the same turn that the fork is gone, gets the `turn_id` of the last successfully fork-applied revision, and gets a propagation directive ("mention this in your response so the user — or your parent agent if you're a Task subagent — knows"). So the release isn't silent: the AI can guide you back via `recall(turn_id="<last-fork-applied>")` to inspect or `rewind_to(turn_id="<last-fork-applied>", message="...")` to resume the fork content with a fresh prompt.
