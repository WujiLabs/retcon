...
- **All LLM calls claude makes — including the summarization that drives /compact — route through `ANTHROPIC_BASE_URL`.** No special-cased internal endpoint, no separate "maintenance" model that bypasses the proxy. This is what makes /compact's summary represent the forked branch instead of claude's pre-compact local view, and is therefore what makes the post-compact alignment described in §6 hold. If a future harness shipped a side-channel summarization call, /compact would still fire its hook (so retcon would still release the override correctly), but the summary would be of the wrong conversation and the user would notice the discontinuity. Worth a targeted assumption test: run a forked branch, trigger compaction, assert that an extra `/v1/messages` event landed in our log just before the `compact` hook.
